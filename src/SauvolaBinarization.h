...
namespace ImageBinarization
{
    ...
    {
    public:
        static void Binarize(const cv::Mat &src, cv::Mat &dst, int kernelSize, double k, double r);
    };

    inline void SauvolaBinarization::Binarize(const cv::Mat &src, cv::Mat &dst, int kernelSize, double k, double r = 128)
    {
        if (kernelSize < 3)
        ...
        for (auto i = 0; i < dst.rows; i++)
        {
            ...
            for (auto j = 0; j < dst.cols; j++)
            {
                ...
                auto th = mean *(1.0 + k * (sigma / r - 1.0));
